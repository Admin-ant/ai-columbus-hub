...
export function PdfTemplateDialog({ orgId, onChange }: Props) {
  const [open, setOpen] = useState(false);
  const [tpl, setTpl] = useState<PdfTemplate>(DEFAULT_TEMPLATE);

  useEffect(() => {
    if (orgId) setTpl(loadTemplate(orgId));
  }, [orgId]);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Pas logo, titel, paginanummering en kleurthema aan. Instellingen worden lokaal per omgeving bewaard.
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter className="gap-2">
          <Button type="button" variant="ghost" onClick={reset}>Standaard</Button>
